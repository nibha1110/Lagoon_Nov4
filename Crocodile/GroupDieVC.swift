...
class GroupDieVC: UIViewController, responseProtocol, CommonClassProtocol {
...
//        }
    ...
    //MARK: - AddToDie Button
    @IBAction func AddToDie_btnAction(sender: AnyObject) {
        str_TotalAnimal = "\(str_Animal1)\(str_Animal2)\(str_Animal3)"
       
            let alertView = UIAlertController(title: nil, message: "Are You Sure You Want To Die #\(str_TotalAnimal) Animal From #\(str_MovePen1)-\(str_MovePen2)\(str_MovePen3)?", preferredStyle: .Alert)
    ...
    func GroupedPenService()
    {
        ...
        let postString = "pen_type=Grouped&noofanimals=\(str_TotalAnimal)&groupcode=\(str_MovePen1)&namexx=\(str_MovePen2)&nameyy=\(str_MovePen3)&dateadded=\(str_todatDate)&movedby=\(NSUserDefaults.standardUserDefaults().objectForKey("email_username") as! String)&app_id=\(NSUserDefaults.standardUserDefaults().objectForKey("ApplicationIdentifier") as! String)"
        ...
        do
        {
            ...
            if results.count > 0
            {
                if let objTable: AnimalsCountTable = results[results.count-1] as? AnimalsCountTable {
                    ...
                    let animal = str_TotalAnimal.toInteger()!
                    if totalVal >= animal
                    {
...
//                }
    ...
    func InsertToAnimalgroupdie(totalAnimal: String!)
    {
...
//        fetchRequest.resultType = NSFetchRequestResultType.DictionaryResultType
        do {
            ...
            if fetchedResults.count != 0 {
                if let objTable: Animalgroupdie = fetchedResults[0] as? Animalgroupdie {
...
//
            ...
            else
            {
                ...
                var objCoreTable: Animalgroupdie!
                objCoreTable = (NSEntityDescription.insertNewObjectForEntityForName("Animalgroupdie", inManagedObjectContext: self.appDel.managedObjectContext) as! Animalgroupdie)
            ...
            if fetchedResults.count != 0 {
                for i in 0 ..< fetchedResults.count {
                    if let objTable: AnimalsCountTable = fetchedResults[i] as? AnimalsCountTable {
                        ...
                        do {
                            try self.appDel.managedObjectContext.save()
                            self.InsertToAnimalgroupdie(totalAnimal)
                        } catch {
                        }}}}
                    }catch  {
