...
class LoadToTrailerVC: UIViewController, responseProtocol, CommonClassProtocol {
    ...
    override func viewDidLoad() {
        ...
        controller.IIndexPath = NSIndexPath(forRow: 1, inSection: 2)
        controller.tableMenu.reloadData()
        controller.label_header.text = "LOAD TO TRAILER"
        ...
        pickerview1_movePen.selectRow(0, inComponent: 0, animated: true)  //Y2
    ...
    //MARK: - Move Button
    @IBAction func Load_btnAction(sender: AnyObject) {
        str_TotalAnimal = "\(str_Animal1)\(str_Animal2)\(str_Animal3)"
        
        let alertView = UIAlertController(title: nil, message: "Are You Sure You Want To Load #\(str_TotalAnimal) Animals To Trailer?", preferredStyle: .Alert)
    ...
    func LoadToTrailerService()
    {
        appDel.Show_HUD()
        str_webservice = "addtotrailer"
        let request = NSMutableURLRequest(URL: NSURL(string: "\(Server.local_server)/api/trailer/addtotrailer.php?")!)
        let postString = "group=\(str_MovePen1)&namexx=\(str_MovePen2)&nameyy=\(str_MovePen3)&qty=\(str_TotalAnimal)&app_id=\(NSUserDefaults.standardUserDefaults().objectForKey("ApplicationIdentifier") as! String)"
    ...
    func responseDictionary(dic: NSMutableDictionary) {
        ...
        if self.str_webservice == "addtotrailer"
        {
            if dic["success"] as! String == "True"
            {
                let alertView = UIAlertController(title: nil, message: "Lot Added To Trailer Successfully.", preferredStyle: .Alert)
                alertView.addAction(UIAlertAction(title: "OK", style: .Cancel, handler: nil))
                self.presentViewController(alertView, animated: true, completion: nil)
            }
            else
            ...
            if dic["success"] as! String == "True" {
                ...
                msg = "Hatchlings Have Been Moved Successfully."
                
                
            }
            else
            {
                msg = "Hatchlings Not Moved."
            }
